...
This (rather long) document has two purposes: (1) to calculate the initial biomass estimates for init.nc based on 1990 (or closest) biomass estimates from stock assessments, expanded to account for unassessed age classes and for biomass in British Columbia; and (2) to calculate estimates of biomass per box to obtain values for the spatial distributions S1-S4. 
...
make_bc_from_ak_assessment <- function(this_fg_stage){
  ...
  dz <- c(0,30,100,200,500,1000,4000)
...
So these should be all the assessed species. We are left with:
...
So who are the species left for points 1 and 3?
```{r}
...
Drop salmon - the model in AK does not look too bad for chinook and coho but it is probably still too dangerous to use (they should not be caught and there are few data points and they are an important species so better keep them simple than introducing false precision here).

We need the parameters for these too!!
...
no_ak_sdm_no_as <- c('EulachonJ','Forage_slopeJ','HerringJ','Pacific_hakeJ','Shallow_demersalJ','Shark_demersalJ')

# these are those that scored OK in the validation for AK but we do not want to use here because either we are missing a biomass estimate (rockfish demersal shelf) or because they are salmon groups that we are doing from other sources

drop_these <- c("Rockfish_demersal_shelfA", "Salmon_chinookA", "Salmon_chumJ")
...
Minimal difference for sculpins, king crab, filter feeders, and herring (but we do not use the last from the bottom trawl anyways). What these have in common is a very high (>90% of the total biomass predicted by sdmTMB) density in the deeper slope boxes, and we can trace that back to few data points and mainly on the slope. Small differences from some others. However, all groups have a positive difference.

This is not news - we have dedicated SDMs for herring, demersal sharks, and soon for king crab. In addition, even groups whose models converged and were OK should probably no be modeled following bottom trawl data. Forage_slope fish occurred in minimal data points in BC, and was not stellar in AK. We have an alternative that bases those on depth. Benthic carnivores are largely polychaetes, and bottom trawl surveys are really not good at getting those. We have code that maps those based on sediment type instead and is probably more sensible. Sponges are also probably not great here, we have results from SDM studies that do a far better job (although that is cover and not biomass). 
