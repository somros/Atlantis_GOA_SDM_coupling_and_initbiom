...
ak_biomass <- read_xlsx('../../../StockAssessments/biomass_from_stock_assessment.xlsx',sheet = 1,range='A1:AF54')
...
biol_prm <- read_xlsx('../../../StockAssessments/biomass_from_stock_assessment.xlsx',range='A1:J32', sheet = 3) # sculpins and rockfish demersal shelf use average of parameters
...
One exception: adult hake scored low (not enough data points), but for now we keep it in (set to 3 in validation table) because we have nothing better and because the model in BC is OK.
```{r}
...
```{r}
no_ak_sdm <- c('CapelinJ', 'DogfishJ', 'EulachonJ', 'Forage_slopeJ', 'HerringJ', 'Pacific_hakeJ', 'Rockfish_demersal_shelfJ', 'Rockfish_pelagic_shelfJ', 'SculpinsJ', 'Shallow_demersalJ', 'Skate_bigJ', 'Skate_longnoseJ', 'Skate_otherJ')
```
...
```{r}
no_ak_sdm_as <- c('DogfishJ', 'Rockfish_demersal_shelfJ', 'Rockfish_pelagic_shelfJ', 'SculpinsJ', 'Skate_bigJ', 'Skate_longnoseJ', 'Skate_otherJ')
```
...
Examining the output here shows that some groups are problematic. Some notable ones are juvenile pollock and the `Flatfish_shallow` complex. Those will need to be adjusted in post-processing, or they will requre dedicated `sdmTMB` models. 
