...
split_stage_ecopath <- function(this_fg){ #TODO could be made one function with the one above to get unassessed biomass
  
  this_fg <- 'EulachonA'
  this_species <- substr(this_fg,1,(nchar(this_fg)-1))
  
  if(this_species %in% parameters_ecopath$name){
    this_stage <- substr(this_fg,nchar(this_fg),nchar(this_fg))
  }
  
  this_biom <- biomass_ecopath %>% filter(name==this_fg)
...
make_bc_from_ak_ecopath <- function(this_fg_stage){
  # read in AK dists (switch for adult if we do not have juvenile), fill empty boxes
  # read in biomass from ecopath (either for the stage or for the whole if invert)
  # allocate to BC based on dist, get depth avg, allocate to BC based on strata
  # read in BC if we have it (even adult, see switches above)
  # reallocate tot biom based on BC dists
  # done
}
...
**TODO: check that all distributions add up to one at all stages or else Atlantis complains**
